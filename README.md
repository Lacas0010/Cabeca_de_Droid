...
* **Motor de Normalização de Nomes e Resolução de Apelidos:** Conta com um algoritmo que limpa e normaliza os nomes dos personagens (removendo hifens, acentos, bullets e parênteses) antes do cruzamento de IDs, garantindo que guias de metagame em inglês sejam mapeados corretamente para os registros oficiais da conta (ex: mapeamento correto de `"Himeko - Nova"` para `"1510"` e `"Tingyun • Fugue"` para `"1225"`).
