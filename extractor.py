...
def clean_genshin_url(url) -> str:
    """Sanitiza URLs de imagem de Genshin eliminando sufixos bugados e extensoes duplicadas."""
    if not url:
        return ""
    u = str(url).strip()
    u = u.replace("UI_AvatarIcon_ide_", "UI_AvatarIcon_")
    u = u.replace("UI_Gacha_AvatarImg_ide_", "UI_Gacha_AvatarImg_")
    u = u.replace("UI_Costume_ide_", "UI_Costume_")
    while u.endswith(".png.png"):
        u = u[:-4]
    return u
...
class GenshinExtractor(BaseExtractor):
    ...
    async def extrair_e_salvar(self, filename: str = "genshin/roster_genshin.md") -> str:
        ...
        try:
            ...
            for char in sorted(chars, key=lambda c: (getattr(c, 'rarity', 4), getattr(c, 'level', 1)), reverse=True):
                try:
                    c_id_val = str(getattr(char, "id", ""))
                    try:
                        c_name_val = char.name
                    except Exception:
                        c_name_val = f"Personagem {c_id_val}"
                    try:
                        c_level_val = char.level
                    except Exception:
                        c_level_val = 1
                    try:
                        c_rarity_val = char.rarity
                    except Exception:
                        c_rarity_val = 4
                    try:
                        c_const_val = char.constellation
                    except Exception:
                        c_const_val = 0
                    try:
                        c_elem_val = char.element
                    except Exception:
                        c_elem_val = "Anemo"
                    ...
                    if hasattr(char, "weapon") and char.weapon:
                        w_info = {
                            "name": getattr(char.weapon, "name", "Arma"),
                    ...
                    if hasattr(char, "artifacts") and char.artifacts:
                        for art in char.artifacts:
                            ...
                            artifacts_json.append({
                                "name": clean_relic_name(getattr(art, "name", "Artefato")),
                    ...
                    # Limpa e sanitiza URLs de imagem de Genshin
                    char_icon = clean_genshin_url(char_icon)
                    genshin_splash = clean_genshin_url(genshin_splash)
                    ...
                    print(f"Aviso ao processar personagem {getattr(char, 'id', '?')}: {char_err}")
